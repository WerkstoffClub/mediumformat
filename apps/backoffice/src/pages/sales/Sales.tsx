  ...
  return (
    <div className="space-y-3.5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-[24px] font-semibold tracking-[-0.04em] leading-8 text-[var(--text-primary)]">Sales</h1>
          <p className="text-[13px] text-[var(--text-muted)] mt-0.5">
            Revenue, margins, channels and payments — from the DealPOS mirror{lastSync ? ` · last sync ${new Date(lastSync).toLocaleString('en-GB')}` : ''}
          </p>
        </div>
        <button
          onClick={onSync}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input type="date" value={range.from} max={range.to} onChange={e => { setRange(r => ({ ...r, from: e.target.value })); setActivePreset(null); }} className={selectCls} />
        <span className="text-[11px] text-[var(--text-muted)]">to</span>
        <input type="date" value={range.to} min={range.from} onChange={e => { setRange(r => ({ ...r, to: e.target.value })); setActivePreset(null); }} className={selectCls} />
